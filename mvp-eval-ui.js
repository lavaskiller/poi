...
const I18N={
  ko:{appSubtitle:'데이터 상태 확인 → 알고리즘 실행 → 결과·실패 케이스 분석',tabOverview:'개요',tabRun:'알고리즘 실행',tabResults:'실행 결과',tabData:'데이터셋 관리',resultsIntro:'실행 결과는 이곳에서 관리합니다. 실행을 하나 선택해 설정과 실패를 확인하고, 최대 4개 실행을 비교하거나 필요 없는 실행을 삭제하세요.',savedRuns:'저장된 실행',compareSelect:'비교 선택 (최대 4개)',runDetailEmpty:'왼쪽에서 실행을 선택하세요. 설정, 측정 결과, 필터 가능한 실패 케이스를 표시합니다.',selectedCompare:'선택 실행 비교',compareHelp:'막대는 정확도입니다. 아래 색 띠는 정답, 기권, 오류, 오답의 구성입니다. 같은 코드 해시는 같은 제출 코드의 재실행을 뜻합니다.',retrievalDiagnostics:'후보 검색 진단',retrievalDiagnosticsHelp:'- 알고리즘 실행 결과와 별개인 후보 공급원 상태'},
  en:{appSubtitle:'Check data health → run an algorithm → inspect results and failures',tabOverview:'Overview',tabRun:'Run algorithm',tabResults:'Run results',tabData:'Dataset management',resultsIntro:'Manage persisted results here. Select one run to inspect its configuration and failures, compare up to four runs, or remove an obsolete result.',savedRuns:'Saved runs',compareSelect:'Compare (up to 4)',runDetailEmpty:'Select a run on the left. Its configuration, measured outcomes, and filterable failures appear here.',selectedCompare:'Compare selected runs',compareHelp:'Bars show accuracy. The colored strip breaks down correct, abstained, errored, and wrong cases. Matching code hashes mean the same submission was run again.',retrievalDiagnostics:'Retrieval diagnostics',retrievalDiagnosticsHelp:'- candidate-provider health, separate from algorithm results'}
};
...
function renderRunDetail(){
  const r=selectedRun,m=r.metrics||{},n=m.n_eligible||0,wrong=Math.max(0,n-(m.correct||0)-(m.abstained||0));
  const L=uiLanguage==='ko', label=L?{del:'실행 삭제',created:'실행 시각',config:'설정',inputs:'입력',identity:'코드 식별',candidates:'후보',all:'전체',none:'추가 신호 없음',correct:'정답',abstained:'기권',errors:'오류',wrong:'오답',failures:'실패 케이스',total:'전체',matching:'필터 결과',allDatasets:'모든 데이터셋',allOutcomes:'모든 결과',previous:'이전',next:'다음',noFailures:'조건에 맞는 실패가 없습니다.',error:'오류'}:{del:'Delete run',created:'Created',config:'Config',inputs:'Inputs',identity:'Code identity',candidates:'candidates',all:'all',none:'none',correct:'correct',abstained:'abstained',errors:'errors',wrong:'wrong',failures:'Failures',total:'total',matching:'matching',allDatasets:'All datasets',allOutcomes:'All outcomes',previous:'Previous',next:'Next',noFailures:'No failures match these filters.',error:'Error'};
