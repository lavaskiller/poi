...
- [x] category/ID/좌표 보존
- [x] weighted ranker
- [x] confidence tier와 reason code
- [x] auto precision, auto coverage, picker recall 지표
- [x] versioned policy constant (`AUTO_MARGIN_M`)와 case-level JSON report
- [ ] rich MapKit 후보 재수집 (기존 snapshot 다수는 top-3/metadata 부족)
- [ ] threshold 조정 UI (현재는 코드 상수 + CLI report만 제공)

완료 기준:

- [x] 같은 snapshot에서 nearest와 정책 v0 비교
- [x] action/reason별 case-level audit JSON 생성
- [ ] confidence tier별 held-out 실측 정답률 표시 (현재 AUTO 2건이라 신뢰도 추정 불가)
- [ ] 오류 case를 dashboard UI에서 확인 가능

#### MVP 1 구현 관찰 (2026-03)

- `examples/poi_confidence_policy.py`가 `AUTO_PICK / SHOW_PICKER / NONE`과 reason code를 반환한다.
- `tools/simulate_confidence_policy.py`는 GT를 policy input에 넣지 않고 post-hoc으로 risk-coverage를 계산한다.
- 현 private 166-case snapshot에서 policy v0는 AUTO 2/166 (1.2%), auto precision 2/2, wrong-auto 0/166, picker 146/166 (88.0%), picker recall@5 74/146 (50.7%), NONE 18/166 (10.8%)였다.
- 이 2건으로 production auto precision을 주장할 수 없다. AUTO 확장은 richer retrieval 및 held-out calibration 이후에만 한다. 상세 계약과 실행법은 `docs/reports/confidence-policy-simulator-v0.md`를 따른다.
