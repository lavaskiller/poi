...
# Prompt styles for Top-K image reranking. Prefer short, priority-ordered
# instructions: FastVLM-0.5B follows brief skills better than long prose.
PROMPTS = {
    # Original baseline: allow UNKNOWN → nearest fallback in the runner.
    "baseline": """Look at the image and identify its main photographed place.
...
""",
    # Skill-style guidance (PWE-13 oriented). Still allows UNKNOWN for safe fallback.
...
""",
    # Force a choice — for GT-in-list experiments only; never use as product default.
...
""",
    # Photo–place match (product framing): not "which is nearest", but
    # "which place is this photo most clearly taken at".
...
# Back-compat for importers that expect PROMPT_TEMPLATE / build_prompt().
PROMPT_TEMPLATE = PROMPTS["baseline"]


def build_prompt(candidates: List[Dict[str, Any]], style: str = "baseline") -> str:
    lines = [f"{i}. {c.get('name', '').strip()}" for i, c in enumerate(candidates, 1)]
    template = PROMPTS.get(style) or PROMPTS["baseline"]
...
def cache_key(case: Dict[str, Any], candidates: List[Dict[str, Any]], model_path: Path,
              prompt_style: str = "baseline") -> str:
    template = PROMPTS.get(prompt_style) or PROMPTS["baseline"]
    evidence = {
        "dataset": case["_dataset"], "photo": case["_photo"],
        "candidates": [c.get("name", "") for c in candidates],
        "model": model_path.name, "prompt": template, "prompt_style": prompt_style,
        "cache_schema": 3,
    }
...
def main() -> int:
    ...
    p.add_argument("--prompt-style", choices=sorted(PROMPTS), default="baseline",
                   help="baseline=UNKNOWN allowed; skill=priority guide; skill_force=always pick")
    p.add_argument("--cache", default=None,
                   help="JSONL cache path (default: generated/vlm_topk_{style}_k{K}_cache.jsonl)")
    p.add_argument("--results-tsv", default=None)
    p.add_argument("--run-name", default=None)
    p.add_argument("--use-active-snapshot", action="store_true", default=True,
                   help="Load active MapKit snapshot (default true)")
    p.add_argument("--legacy-candidates", action="store_true",
                   help="Use legacy generated/mapkit_candidates.jsonl instead of active snapshot")
    args = p.parse_args()
    ...
    if args.legacy_candidates:
        cand_paths = [str(data_dir / "generated/mapkit_candidates.jsonl")]
    else:
    ...
    default_stem = f"vlm_topk_{style}_k{args.candidate_limit}"
    cache_path = Path(args.cache or (data_dir / "generated" / f"{default_stem}_cache.jsonl"))
    results_tsv = Path(args.results_tsv or (data_dir / f"{default_stem}_results.tsv"))
    run_name = args.run_name or default_stem.replace("_", "-")
        ...
        if item is None:
            ...
            if not cands:
                ...
                try:
                    ...
                    raw = model.infer(image, build_prompt(cands, style=style))
                    selected = parse_selection(raw, cands)
                    if selected is None:
                        # skill_force: if parse failed, still keep nearest rather than empty
    ...
    print(json.dumps({"run": str(run_path), "metrics": record["metrics"],
                      "prompt_style": style, "cache": str(cache_path)}, ensure_ascii=False, indent=2))
    return 0
