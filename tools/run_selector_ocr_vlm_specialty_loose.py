#!/usr/bin/env python3
"""Exploratory OCR + semantic-evidence correction layer over MapKit Bloggo.

This runner accepts only unique direct OCR candidate-name evidence first.  On
all remaining Bloggo-ambiguous cases, FastVLM
...
candidate-name-free FastVLM prompt gives an affirmative response that the photo
depicts that specialty. The affirmative parser accepts normal output-format
variants (``1``, ``yes``, and explanatory ``yes ...``). It never reads ground
truth during prediction.
...
from run_selector_ocr_override import supported_candidates
...
RULE_VERSION = "unique-direct-ocr-or-raw-top5-choice+candidate-name-free-specialty-yes-v5-permissive-exploratory"
CACHE_SCHEMA = 1
...
def permissive_specialty_yes(raw: str) -> bool:
    """Accept common affirmative variants, but never a negative response.

    This relaxes verifier *format*, while retaining the independent,
    candidate-name-free visual verification question. It is an upper-bound
    exploratory replay, not a deployment criterion.
    """
    text = (raw or "").strip().casefold()
    return bool(re.match(r"^(?:1\b|yes\b|y\b|true\b|affirmative\b)", text))
...
def unique_ocr_candidate(ranked: List[Dict[str, Any]], ocr_text: str) -> Optional[Dict[str, Any]]:
    """Return a candidate only for one unambiguous direct OCR match."""
    evidence = supported_candidates(ranked, ocr_text)
    return evidence[0]["candidate"] if len(evidence) == 1 else None
...
def process_case(case: Dict[str, Any], data_dir: Path, cfg: Dict[str, Any], model_repo: Path, model_path: Path,
    ...
    ocr_text = str(case["input"].get("ocr_text") or "")
    ...
    ocr_candidate = unique_ocr_candidate(ranked, ocr_text) if base is not None else None
    if ocr_candidate is not None:
        nominated = ocr_candidate
    ...
    elif base is not None and resolution["decision"] == "ambiguous":
            ...
            try:
                ...
                if index is None:
                    ...
                    if same:
                        ...
                        if permissive_specialty_yes(verification_raw):
                            prediction = str(nominated.get("name") or "").strip()
    ...
    return ({
        ...
        "bloggo_gap_m": resolution.get("gap_m"), "ranked_candidates": json.dumps([str(c.get("name") or "") for c in ranked], ensure_ascii=False), "ocr_text": ocr_text,
...
def main() -> int:
    ...
    parser.add_argument("--verification-cache", default=str(ROOT / "poi-data/generated/fastvlm_bloggo_ocr_fastvlm_semantic_v5_permissive_cache.jsonl")); parser.add_argument("--results-tsv", default=str(ROOT / "poi-data/fastvlm_bloggo_ocr_fastvlm_semantic_v5_permissive_results.tsv"))
    parser.add_argument("--run-name", default="selector-ocr-vlm-specialty-loose")
    ...
    cases = ra.build_cases(ms.read_rows(str(data_dir / "eval_set_reconciled.csv")), cfg, ms.load_candidates([str(data_dir / "generated/mapkit_candidates.jsonl")]), "all", ["image", "ocr_text", "nearby_candidates"], args.candidate_limit)
    ...
    record = {"name": args.run_name, "safe_name": safe_name, "version": version, "created_at": datetime.datetime.now().isoformat(timespec="seconds"), "scope": "all" if args.limit is None else f"first-{args.limit}", "mode": "exact", "params": ["image", "ocr_text", "nearby_candidates"], "candidate_limit": args.candidate_limit, "lang": "fastvlm-mps-bloggo-ocr-semantic", "rule_version": RULE_VERSION, "script_sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(), "evaluation_set_sha256": ra.evaluation_set_sha256(cases), "data_snapshot_sha256": ra.data_snapshot_sha256([str(data_dir / "eval_set_reconciled.csv"), str(data_dir / "dashboard_config.json"), str(data_dir / "generated/mapkit_candidates.jsonl"), str(ROOT / "examples/mapkit_weighted.py"), str(ROOT / "examples/poi_confidence_policy.py")]), "metrics": {key: value for key, value in scored.items() if key != "cases"}, "cases": scored["cases"]}
